<?php
// Commencer ou reprendre une session
session_start();

// Vérifier si l'utilisateur est connecté en vérifiant si la variable 'username' existe dans la session.
// Si ce n'est pas le cas, rediriger l'utilisateur vers la page de connexion.
if (!isset($_SESSION['username'])) {
    header('location:login/');
}

// Inclure le fichier 'db.php' qui contient probablement les informations de connexion à la base de données.
include('../include/db.php');

// Écrire une requête SQL pour obtenir toutes les données de tous les utilisateurs.
$query = "SELECT * FROM user";

// Exécuter la requête SQL.
$queryrun = mysqli_query($db, $query);

// Récupérer les données du premier utilisateur renvoyé par la requête.
$data = mysqli_fetch_array($queryrun);

// Écrire une deuxième requête SQL pour obtenir les données de l'utilisateur actuellement connecté.
                ...
                <form method="post">
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="firstname" value="<?= $data2['firstname'] ?>" class="form-control" id="ptitle" placeholder="Elyesse">
                        </div>

                        <div class="form-group col-md-6">
                            <label for="ptitle">Nom</label>
                            <input type="text" name="lastname" value="<?= $data2['lastname'] ?>" class="form-control" id="ptitle" placeholder="Kourdourli">
                        </div>

                        <div class="form-group col-md-12">
                            <label for="psubtitle">Email ID</label>
                            <input type="text" name="email" value="<?= $data2['email'] ?>" class="form-control" id="psubtitle" placeholder="admin@admin.com">
                        </div>
                    </div>
